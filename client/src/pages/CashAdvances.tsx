...
import { useRestoreCashAdvance, useVoidedCashAdvances } from '../hooks/useCashAdvances';
import { CashAdvanceFilters, type CashAdvanceFilterValues } from '../components/CashAdvanceFilters';
...
import type { CashAdvance, CashAdvanceListResponse, CashAdvanceStatus } from '../types';
...
const columns: ColumnDef<CashAdvance>[] = [
  ...
  { key: 'requested_by', label: 'Requested by', cardTitle: true, render: (value) => (value as string) ?? '—' },
  { key: 'purpose', label: 'Purpose', cardSubtitle: true, render: (value) => (value as string) ?? '—' },
  { key: 'control_no', label: 'Control No.', render: (value) => (value as string) ?? '—' },
  {
  ...
  },
  {
    ...
    label: 'Status',
    render: (value) => (
      <StatusPill tone={STATUS_TONE[value as CashAdvanceStatus]}>{STATUS_LABEL[value as CashAdvanceStatus]}</StatusPill>
    ),
  },
  {
    key: 'needs_review',
...
export function CashAdvances() {
  ...
  const [filters, setFilters] = useState<CashAdvanceFilterValues>({ date_from: '', date_to: '' });
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h2 className="font-display text-xl font-semibold text-ink">Cash Advances</h2>
      ...
      {modal && (
        <Modal title={modal === 'create' ? 'New cash advance' : 'Edit cash advance'} onClose={handleModalClose}>
          <CashAdvanceForm cashAdvance={modal === 'create' ? undefined : modal} onClose={handleModalClose} />
        </Modal>
      )}
          ...
          renderRow={(ca) => (
            <>
              <div className="font-medium">
                {ca.txn_date} — {ca.requested_by ?? 'No requester'} — {formatMoney(ca.amount)}
              </div>
